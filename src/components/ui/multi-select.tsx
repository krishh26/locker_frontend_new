...
function removePickedOption(options: Option[], picked: Option[]): Option[] {
  return options.filter(option => !picked.find(p => p.value === option.value))
}

function isOptionsExist(options: Option[], targetOption: Option[]): boolean {
  return options.some(option => targetOption.find(p => p.value === option.value))
}
...
const MultipleSelector = ({
  ...
  const selectables = React.useMemo<Option[]>(() => removePickedOption(options, selected), [options, selected])
          ...
          {selected.map(option => {
            return (
              <div
                key={option.value}
                className={cn(
                  'animate-fadeIn bg-muted text-foreground hover:bg-muted relative inline-flex h-7 cursor-default items-center rounded-md border border-input pr-7 pl-2 text-xs font-medium transition-all disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 data-fixed:pr-2',
                  badgeClassName
                )}
